...
def find_key_event_time(parsed_logs, key_info):
    ...
    if event_time is not None:
        ...
        if GLOBAL_MAX_TIME is not None:
            event_time = GLOBAL_MAX_TIME
        else:
            if parsed_logs:
                event_time = parsed_logs[-1].get('TIME_seconds', 0)
            else:
                event_time = 0
            print(f"Warning: 找不到关键事件时间，已将 tracking_end_time 设为日志最后时间点: {event_time}s")
        return event_time
...
if __name__ == "__main__":
    ...
    #========== Prepare logs =================#
    logdir = '/data2/AAAI5026_new/FractFlow-main/tools/aircraft/tmp/logs/flight_agent_20260122_114521'
